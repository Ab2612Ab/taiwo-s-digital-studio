...
function conversationReply(input: string, history: Message[]) {
  ...
  if (/mobile|responsive|phone|tablet/.test(text)) return "Responsive behaviour is part of the portfolio approach. Which screen or user journey is giving you trouble: mobile, tablet, desktop, or all of them?";
  if (/help me plan|don't know|do not know|what features|features/.test(text)) return "Absolutely. We can plan it together without deciding everything at once. First, what does the business need the website to help customers do: learn, enquire, book, buy, or something else?";
...
export function AIStudioAssistant() {
  ...
  return <div className="fixed bottom-24 right-5 z-[60]">
    {open && <div className="mb-3 flex h-[min(620px,calc(100vh-120px))] w-[min(400px,calc(100vw-32px))] flex-col overflow-hidden rounded-3xl border border-border bg-background/95 shadow-2xl backdrop-blur-xl">
      <div className="flex items-center justify-between border-b border-border p-4"><div className="flex items-center gap-3"><div className="grid size-10 place-items-center rounded-full bg-primary text-primary-foreground"><Bot className="size-5"/></div><div><p className="font-semibold">Taiwo's Studio Assistant</p><p className="text-xs text-muted-foreground">{listening ? "Listening..." : processing ? "Processing..." : speaking ? "Speaking..." : "Conversational help"}</p></div></div><button type="button" aria-label="Close assistant" onClick={()=>{setOpen(false);stopSpeaking();}} className="rounded-full p-2 hover:bg-muted"><X className="size-4"/></button></div>
      <div className="flex-1 space-y-3 overflow-y-auto p-4">{messages.map(message=><div key={message.id} className={message.from==="user"?"ml-8 rounded-2xl rounded-br-md bg-primary p-3 text-sm text-primary-foreground":"mr-8 rounded-2xl rounded-bl-md bg-muted p-3 text-sm text-foreground"}><div>{message.text}</div>{message.from==="bot" && voiceOutputAvailable && <div className="mt-2 flex gap-1"><button type="button" onClick={()=>speak(message.text)} className="inline-flex items-center gap-1 rounded-full border border-border px-2 py-1 text-[11px] hover:border-primary/50" aria-label="Listen to assistant response"><Volume2 className="size-3"/> Listen</button><button type="button" onClick={stopSpeaking} className="inline-flex items-center gap-1 rounded-full border border-border px-2 py-1 text-[11px] hover:border-primary/50" aria-label="Stop assistant speech"><VolumeX className="size-3"/> Stop</button></div>}</div>)}{processing && <div className="mr-8 rounded-2xl bg-muted p-3 text-sm text-muted-foreground animate-pulse">Processing...</div>}{speaking && <div className="mr-8 rounded-2xl border border-border bg-background p-2 text-xs text-muted-foreground">Speaking...</div>}<div className="flex flex-wrap gap-2 pt-1">{suggestions.map(s=><button key={s} type="button" onClick={()=>send(s)} className="rounded-full border border-border px-3 py-1.5 text-xs hover:border-primary/50">{s}</button>)}</div></div>
      <form onSubmit={e=>{e.preventDefault();send();}} className="border-t border-border p-3"><div className="flex items-end gap-2"><textarea value={input} onChange={e=>setInput(e.target.value)} rows={1} maxLength={4000} placeholder={listening?"Listening...":processing?"Processing...":"Tell me about your project..."} aria-label="Message the website assistant" className="min-h-10 max-h-28 min-w-0 flex-1 resize-y rounded-2xl border border-input bg-background px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-primary"/><button type="button" onClick={toggleListening} disabled={!voiceInputAvailable || processing} aria-label={voiceInputAvailable?(listening?"Stop recording and send":"Tap to speak"):"Voice input is unavailable in this browser"} title={!voiceInputAvailable?"Voice input is unavailable in this browser":listening?"Stop recording and send":"Tap to speak"} className={`grid size-10 shrink-0 place-items-center rounded-full border border-border ${listening?"bg-primary text-primary-foreground animate-pulse":"bg-background"} disabled:cursor-not-allowed disabled:opacity-40`}>{listening?<MicOff className="size-4"/>:<Mic className="size-4"/>}</button><button type="submit" disabled={processing} aria-label="Send message" className="grid size-10 shrink-0 place-items-center rounded-full bg-primary text-primary-foreground disabled:opacity-50"><Send className="size-4"/></button></div>{!voiceInputAvailable && <p className="mt-2 text-[11px] text-muted-foreground">Voice input is unavailable in this browser. You can type your message instead.</p>}{!voiceOutputAvailable && <p className="mt-1 text-[11px] text-muted-foreground">Spoken replies are unavailable in this browser; text chat still works normally.</p>}<div className="mt-2 flex items-center justify-between"><button type="button" onClick={resetConversation} className="text-[11px] text-muted-foreground underline-offset-2 hover:underline">Reset conversation</button><div className="flex gap-2"><a href="/booking" className="rounded-full bg-primary px-3 py-1.5 text-xs font-medium text-primary-foreground">Book a Project</a><a href="/contact" className="rounded-full border border-border px-3 py-1.5 text-xs font-medium">Contact me</a></div></div></form>
    </div>}
