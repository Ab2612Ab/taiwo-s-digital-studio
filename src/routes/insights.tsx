...
export const ARTICLES = [
  { slug: "what-makes-a-business-website-actually-work", title: "What Makes a Business Website Actually Work?", category: "Business", date: "September 12, 2026", reading: "6 min", description: "Discover how a focused website can earn trust, guide decisions and turn more of the right visitors into customers.", image: designMistakes, imageAlt: "Realistic modern creative workspace with a professional website displayed on a desktop monitor" },
  ...
  { slug: "good-ui-ux-turns-visitors-into-customers", title: "How Good UI/UX Turns Visitors Into Customers", category: "UI/UX", date: "August 24, 2026", reading: "7 min", description: "Clarity, hierarchy and reduced friction can make a website easier to use and easier to act on.", image: uxConversion, imageAlt: "UX designer reviewing a digital interface in a modern studio" },
  { slug: "before-building-a-business-website", title: "What Every Business Should Know Before Building a Website", category: "Business", date: "August 17, 2026", reading: "6 min", description: "The questions worth answering about goals, audience, content, ownership and maintenance before development begins.", image: buildingWebsite, imageAlt: "Web professional planning a business website in a modern office" },
  { slug: "designing-websites-that-build-trust", title: "Designing Websites That Build Trust", category: "Web Design", date: "August 10, 2026", reading: "5 min", description: "Trust is built through details: consistency, useful information, honest messaging and a predictable experience.", image: trust, imageAlt: "Professional workspace with a laptop and modern office setting" },
  { slug: "from-idea-to-launch-website-development-process", title: "From Idea to Launch: My Website Development Process", category: "Development", date: "August 3, 2026", reading: "7 min", description: "A transparent view of how a website project moves from discovery and structure through development, testing and launch.", image: process, imageAlt: "Web developer working at a modern coding workstation" },
] as const;
...
function ArticleCard({ article }: { article: (typeof ARTICLES)[number] }) {
  return <article className="group flex h-full flex-col overflow-hidden rounded-2xl border border-border bg-card/60 transition-all duration-300 hover:-translate-y-1 hover:shadow-[var(--shadow-elegant)]">
    <Link to="/insights/$slug" params={{ slug: article.slug }} aria-label={`Read ${article.title}`} className="block overflow-hidden border-b border-border bg-muted">
      <img src={article.image} alt={article.imageAlt} loading="lazy" className="aspect-[16/9] w-full object-cover transition-transform duration-500 group-hover:scale-[1.03]" />
    </Link>
...
function InsightsPage() {
  ...
  return <div className="min-h-screen bg-background text-foreground"><SiteHeader /><main className="px-5 pb-24 pt-32 text-center md:pt-40"><div className="mx-auto max-w-6xl">
    ...
    <Reveal delay={80}><section className="mx-auto mt-14 max-w-5xl overflow-hidden rounded-3xl border border-border bg-card/60"><div className="grid lg:grid-cols-[1.15fr_.85fr]"><Link to="/insights/$slug" params={{ slug: featured.slug }} className="block overflow-hidden bg-muted"><img src={featured.image} alt={featured.imageAlt} width={1600} height={1000} className="h-full min-h-64 w-full object-cover transition-transform duration-500 hover:scale-[1.02]" /></Link><div className="flex flex-col items-center justify-center p-7 text-center sm:p-10"><p className="text-xs font-semibold uppercase tracking-[0.18em] text-primary">Featured Insight</p><p className="mt-5 text-xs text-muted-foreground">{featured.category} · {featured.date} · {featured.reading}</p><h2 className="mt-3 text-2xl font-bold leading-tight sm:text-3xl">{featured.title}</h2><p className="mt-4 max-w-xl leading-7 text-muted-foreground">{featured.description}</p><Link to="/insights/$slug" params={{ slug: featured.slug }} className="mt-7 inline-flex items-center gap-2 rounded-full px-5 py-2.5 text-sm font-semibold text-primary-foreground" style={{ background: "var(--gradient-primary)" }}>Discover what makes it work <ArrowRight className="size-4" aria-hidden="true" /></Link></div></div></section></Reveal>
