...
function InsightsPage() {
  ...
  return <div className="min-h-screen bg-background text-foreground"><SiteHeader /><main className="px-5 pb-24 pt-32 text-center md:pt-40"><div className="mx-auto max-w-6xl">
    ...
    <Reveal delay={80}><section className="mx-auto mt-14 max-w-4xl rounded-3xl border border-border bg-card/60 p-8 text-center sm:p-12"><p className="text-xs font-semibold uppercase tracking-[0.18em] text-primary">Featured Insight</p><h2 className="mx-auto mt-4 max-w-3xl text-2xl font-bold leading-tight sm:text-3xl">{ARTICLES[0].title}</h2><p className="mx-auto mt-4 max-w-2xl leading-7 text-muted-foreground">{ARTICLES[0].description}</p><Link to="/insights/$slug" params={{ slug: ARTICLES[0].slug }} className="mt-7 inline-flex items-center gap-2 rounded-full px-5 py-2.5 text-sm font-semibold text-primary-foreground" style={{ background: "var(--gradient-primary)" }}>Discover what makes it work <ArrowRight className="size-4" aria-hidden="true" /></Link></section></Reveal>
